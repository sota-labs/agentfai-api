...
export class EnvUtils {
  ...
  static envString(key: string, defaultValue: string = '') {
    return formatValue(key, defaultValue);
  }

  static envNumber(key: string, defaultValue: number = 0) {
  ...
  static envBoolean(key: string, defaultValue: boolean = false) {
    return formatValue(key, defaultValue, (value) => {
      try {
